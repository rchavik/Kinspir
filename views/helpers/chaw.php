...
class ChawHelper extends AppHelper {
	...
	function params($data = array()) {
		...
		return compact('project', 'fork', 'username');
	}
	...
	function base($params = array()) {
		$params = array_merge(
			array('username' => null, 'project' => null, 'fork' => null), $this->params, $params
		);
	...
	function breadcrumbs($path, $slug = null, $options = array()) {
		...
		$rss = null;
		if ($path && $slug !== 'home') {
			$out[] = $this->Html->link('home', array('controller' => 'projects', 'action' => 'index', '/'));
			$rss = $options['ending'] . $this->rss('home', array('controller' => 'projects', 'action' => 'index', '/', $path, 'ext' => 'rss'));
		}
