...
class ProjectPermission extends AppModel {
	...
	function saveFile($config = array()) {
		...
		if (empty($this->data['ProjectPermission']['fine_grained']) && empty($this->data['ProjectPermission']['username'])) {
			return false;
		}

		if (empty($this->data['ProjectPermission']['fine_grained'])) {
			$repo = $config['url'] . ':/';
			...
			if ($config['repo']['type'] == 'svn') {
				$perms = $this->find('all', array(
					'fields' => array('ProjectPermission.group', 'User.username'),
					'conditions' => array('ProjectPermission.project_id' => $config['id']),
					'recursive' => 0
				));
				$groups = array();
				if (!empty($perms)) {
					foreach ($perms as $perm) {
						$groups[$perm['ProjectPermission']['group']][] = $perm['User']['username'];
					}
				} else {
			...
			$username = $this->data['ProjectPermission']['username'];
			ob_start();
			include(CONFIGS . 'templates' . DS . $config['repo']['type'] . DS . 'permissions.ini');
			$this->data['ProjectPermission']['fine_grained'] = ob_get_clean();
		}
		$result = $File->write(trim($this->data['ProjectPermission']['fine_grained']));
		$this->data = array();
		$this->__rules = array();
		return $result;
	}
