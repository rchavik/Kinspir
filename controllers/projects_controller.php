 ...
 * @package default
 */
class ProjectsController extends RepoAppController {
	...
	function remove($id) {
		...
		if (empty($project)) {
			...
			if ($this->Project->ProjectPermission->deleteAll(array('ProjectPermission.project_id' => $id, 'ProjectPermission.user_id' => $this->Auth->user('id')))) {
				$this->Session->setFlash(sprintf(__("%s was removed ", true), $project['Project']['name']));
			}
			$this->Session->write('Auth.User.ProjectPermission', $this->Project->User->groups($this->Auth->user('id')));
		}
		$this->redirect($this->referer());
	}
			...
			if ($this->data['Project']['id'] != 1) {
				...
				if ($this->Project->initialize() && $this->Project->current['id'] != 1) {
					if ($this->Project->delete($this->data['Project']['id'])) {
						$this->Project->ProjectPermission->deleteAll(array('ProjectPermission.project_id' => $this->data['Project']['id']));
