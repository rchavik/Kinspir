...
class SoftDeletableBehavior extends ModelBehavior {
	...
	function setup(&$Model, $settings = array()) {
		$default = array('field' => 'is_deleted', 'field_date' => 'updated', 'delete' => true, 'find' => true);
	...
	function beforeDelete(&$Model, $cascade = true) {
		if ($this->settings[$Model->alias]['delete'] && $Model->hasField($this->settings[$Model->alias]['field'])) {
			...
			$deleted = $Model->updateAll(
					array(
						$Model->alias . '.' . $this->settings[$Model->alias]['field'] => 1,
						$Model->alias . '.' . $this->settings[$Model->alias]['field_date'] => "'{$date}'"
					), 
...
//			if ($Model->depth == 1) {
//				
//				foreach ($Model->getAssociated('hasMany') as $id => $ModelName) {
//					$queryData['conditions'][$Model->{$ModelName}->alias . '.' . $this->settings[$Model->{$ModelName}->alias]['field'] . ' !='] = '1';
//				}
//				debug($queryData);die;
//
//			} else {
...
//			}
	...
	function beforeSave(&$Model) {
		...
		if ($Model->hasField($this->settings[$Model->alias]['field'])) {
			$Model->data[$Model->alias][$this->settings[$Model->alias]['field']] = 0;
		}
